...
@section('content')
    <div class="max-w-2xl mx-auto">
        <div
            ...
            <form method="POST" action="{{ route('kategoris.update', $kategori->hashid) }}">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="keterangan" rows="4"
                            class="w-full border border-gray-200 dark:border-gray-700 dark:border-gray-700 dark:border-gray-700 dark:border-gray-700 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-amber-400 transition"
                            placeholder="Deskripsi atau catatan tambahan (opsional)">{{ old('keterangan', $kategori->keterangan) }}</textarea>
                        @error('keterangan')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
